...
// --- PIN DEFINITIONS ---
const int irPin = 3;             // RPM Sensor Signal Pin (or NPN sensor via optocoupler)
...
void loop() {
  ...
  if (currentMillis - previousPrintMillis >= printInterval) {
    ...
    float rawRPM = 0; // Variable to hold the raw hardware math

    if (currentPulses > 1) {
      unsigned long timeSpan = endT - startT;
      rawRPM = ((currentPulses - 1) * 60000000.0) / timeSpan;
      
      // Apply the EMA Filter to the raw data
