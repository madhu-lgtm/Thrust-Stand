...
<head>
  ...
  <style>
    ...
    /* --- PROP ICON STYLES WITH SLOWER SPIN CLASS --- */
    .prop-icon {
      width: 38px;
      height: 38px;
    ...
    .prop-icon.spinning {
      animation: spin 0.8s linear infinite; /* Slowed down to 0.8s for a heavier, realistic look */
    }
...
<body>
  ...
  <script>
    ...
    // --- TOGGLE RECORDING & START/STOP SPINNING ---
    function toggleRecord() {
      ...
      if (!isRecording) {
        ...
        prop.classList.add('spinning'); // Triggers the SVG spin animation
        ...
        prop.classList.remove('spinning'); // Stops the SVG spin animation
      }
    }
    ...
    function restartSession() {
      ...
      fetch('/reset').then(r => r.text()).then(res => {
        ...
        btn.innerHTML = '▶ Start Recording';
        btn.className = 'btn btn-record-start';
        prop.classList.remove('spinning'); // Ensure spin stops on reset
