...
// ==========================================
// --- FUNCTION PROTOTYPES (UNO R4 FIX) ---
// ==========================================
void measurePulse();
void measurePWM();
        ...
        if (c == '\n') {
          if (currentLine.length() == 0) {
            ...
            if (requestType == 1) {
              ...
              // -----------------------------------------------------------------
              // PURE HTML/JS/CSS PAYLOAD (AUTO-NORMALIZING CSV LOGIC ADDED)
              // -----------------------------------------------------------------
              client.print(R"=====(
<!DOCTYPE html>
<html>
<head>
  ...
  <style>
    ...
    /* Logo Container */
    .logo-container { text-align: center; margin-bottom: 40px; padding-bottom: 20px; border-bottom: 1px solid #34495e; }
...
<body>
  ...
  <script>
    ...
    // Independent recording variables to normalize CSV data
    let recordIndex = 1;
    ...
    function fetchData() {
      fetch('/data').then(r => r.text()).then(t => {
        ...
        // --- NORMALIZED CSV RECORDING ---
        if (isRecording) {
          // If this is the exact moment we started, log the starting time
          if (recordStartTime === -1) {
            recordStartTime = rawTime;
          }
          
          // Calculate perfectly normalized time starting from 0.0s
          let normalizedTime = (rawTime - recordStartTime).toFixed(1);
          
          // Extract the raw numerical values
          let dataValues = p.map(item => item.split(':')[1].trim());
          
          // Overwrite the Arduino's S.No and Time with our perfect normalized versions
          ...
          if(csvLog.length === 1 || cleanRow !== csvLog[csvLog.length - 1]) {
            ...
            recordIndex++; // Advance our custom index
          }
        }
    ...
    function toggleRecord() {
      ...
      if (!isRecording) {
        // START RECORDING (Wipe old unsaved data, reset internal indexes)
        isRecording = true;
