        ...
        if (c == '\n') {
          if (currentLine.length() == 0) {
            ...
            if (requestType == 1) {
              ...
              // -----------------------------------------------------------------
              // PURE HTML/JS/CSS PAYLOAD (TOP NAVBAR LAYOUT)
              // -----------------------------------------------------------------
              client.print(R"=====(
<!DOCTYPE html>
<html>
<head>
  ...
  <style>
    ...
    .nav-tab.active, .nav-tab:hover { background: rgba(255,255,255,0.1); color: #fff; }
    ...
    /* Grid items are naturally wider now because the main content spans 100% width */
    ...
    .card { 
      ...
      height: 240px; /* Taller cards for bigger graphs */
...
<body>
  ...
  <div class="navbar">
    <div class="nav-left">
      ...
      <div class="nav-tabs">
        <a class="nav-tab active">Live Dashboard</a>
        </div>
    </div>
    ...
    <div class="nav-right">
      ...
      <div class="data-stats">
        Points: <span id="log_count">0</span>
      </div>
    </div>
  </div>
  ...
  <script>
    ...
    function fetchData() {
      fetch('/data').then(r => r.text()).then(t => {
        ...
        drawSparkline('tht', d.tht, '#e74c3c', 0, 50000);     
