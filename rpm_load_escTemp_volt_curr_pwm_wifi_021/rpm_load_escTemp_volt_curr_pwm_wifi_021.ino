...
// --- FAST ASYNCHRONOUS LOAD CELL VARIABLE ---
...
void loop() {
  ...
  // ==========================================
  // We quietly check the HX711 sensor as fast as possible in the background.
  // This prevents the 500ms DAQ loop from ever pausing to wait for the load cell to "wake up".
  if (scale.is_ready()) {
...
<head>
  ...
  <style>
    ...
    .nav-center-text { color: #ffffff; font-size: 26px; font-weight: 700; letter-spacing: 3px; text-transform: uppercase; }
...
<body>
  ...
  if (currentMillis - previousPrintMillis >= printInterval) {
    ...
    // --- MATHEMATICAL TIME LOCK ---
    // Instead of using 'currentMillis', we add exactly 500 to the previous print clock.
    // This physically prevents slipping and ensures mathematically perfect 0.5s increments!
    previousPrintMillis += printInterval; 
    
    // ANTI-RUSH FEATURE:
    // If the WiFi blocked the Arduino for more than 1 full second, fast-forward the clock
    // so we don't start wildly rapid-firing samples to catch up.
    if (currentMillis - previousPrintMillis >= printInterval) {
    ...
    // --- MATHEMATICAL TIME OUTPUT ---
    // Uses the strictly incrementing 'previousPrintMillis' schedule to output perfect time stamps
