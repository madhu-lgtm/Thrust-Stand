...
<head>
  ...
  <style>
    ...
    .nav-center-text { color: #ecf0f1; font-size: 26px; font-weight: 700; letter-spacing: 3px; text-transform: uppercase; text-shadow: 0px 2px 4px rgba(0,0,0,0.3); }
...
<body>
  ...
  <script>
    ...
    function fetchData() {
      fetch('/data?t=' + Date.now()).then(r => r.text()).then(t => {
        ...
        let arduinoIndex = parseInt(p[0]) || 1;
        
        // --- MATHEMATICAL UPTIME LOCK ---
        // This forces the UI Uptime Badge to show perfect 0.5s increments
        let exactUptime = Math.max(0, (arduinoIndex - 1) * 0.5).toFixed(1);
        document.getElementById('status_text').innerText = "Live | Uptime: " + exactUptime + "s";
        
        if (isRecording) {
          if (arduinoIndex !== lastArduinoIndex) {
            lastArduinoIndex = arduinoIndex;
            
            // CSV mathematical time lock
            let exactCsvTime = ((recordIndex - 1) * 0.5).toFixed(1);
            
            let dataValues = [...p]; 
            dataValues[0] = recordIndex;     
            dataValues[1] = exactCsvTime;  
    ...
    function restartSession() {
      ...
      fetch('/reset?t=' + Date.now()).then(r => r.text()).then(res => {
        ...
        // Hard reset the UI Uptime back to 0.0s immediately
        document.getElementById('status_text').innerText = "Live | Uptime: 0.0s";
      });
    }
  </script>
</body>
  ...
  if (currentMillis - previousPrintMillis >= printInterval) {
    ...
    // --- MATHEMATICALLY LOCKED ARDUINO TIMER ---
    ...
    // --- EXACT ARDUINO RAW LOGGING MATH ---
    float runTimeSeconds = (logIndex - 1) * 0.5;
