...
// ==========================================
// --- MAUCH SENSOR CALIBRATION (NOW EDITABLE) ---
// ==========================================
...
<head>
  ...
  <style>
    ...
    .calib-card h2 { margin-top: 0; color: #2c3e50; border-bottom: 1px solid #eee; padding-bottom: 10px; }
    ...
    .desc { color: #95a5a6; font-size: 14px; margin-bottom: 15px; }
  </style>
</head>
<body>
  ...
  <div class="main-content">
    ...
    <div class="calib-container" id="view_calib">
      <div class="calib-card">
        <h2>⚖️ Load Cell Tare</h2>
        <p class="desc">Ensure the thrust stand is completely empty and the motor is NOT running. Clicking Tare will permanently zero out the current load cell reading.</p>
        <button class="btn btn-download" id="btn_tare" onclick="tareLoadCell()">Tare to Zero</button>
      </div>

      <div class="calib-card">
        <h2>⚡ Mauch Sensor Multipliers</h2>
        <p class="desc">Adjust these values to match the calibration sheet provided with your Mauch Power Module.</p>
        
        <div class="input-group">
          <label>Voltage Multiplier (V/V)</label>
          <input type="number" id="inp_vmult" value="19.02" step="0.01">
        </div>
        
        <div class="input-group">
          <label>Current Multiplier (A/V)</label>
  ...
  <script>
    ...
    function fetchData() {
      fetch('/data').then(r => r.text()).then(t => {
        ...
        // --- TURN STATUS BADGE RED ON ERROR ---
        let statusBox = document.getElementById('status_container');
      ...
      fetch('/reset').then(r => r.text()).then(res => {
        ...
        document.getElementById('status_text').innerText = "Session Reset!";
            ...
            if (currentLine.startsWith("GET /data")) { requestType = 1; }
              ...
              requestType = 4; 
              // Extract the numbers sent from the Browser!
