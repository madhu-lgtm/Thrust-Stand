...
// --- ASYNCHRONOUS LOAD CELL STORAGE ---
...
void setup() {
  ...
  // Configure DallasTemp to be non-blocking
  tempSensor.begin();
...
void loop() {
  ...
  if (currentSerialState && !lastSerialState) {
    ...
    Serial.println("S.No,Time(s),Throttle(%),PWM(us),RPM,Thrust(g),Current(A),Voltage(V),Power(W),Temp(C)");
  }
  lastSerialState = currentSerialState;

  // ==========================================
  // --- ZERO-DELAY LOAD CELL POLLING ---
  // ==========================================
  // We instantly poll the HX711 sensor in the background. 
  // This prevents the 500ms DAQ loop from ever pausing to wait for hardware!
  if (scale.is_ready()) {
    float tempThrust = scale.get_units(1);
    currentThrust = (tempThrust < 0) ? 0 : tempThrust;
  }
...
<head>
  ...
  <style>
    ...
    .nav-center { flex: 1; display: flex; align-items: center; justify-content: center; gap: 15px; }
    .nav-center-text { color: #ecf0f1; font-size: 24px; font-weight: 600; letter-spacing: 3px; text-transform: uppercase; text-shadow: 0px 2px 4px rgba(0,0,0,0.3); }
    
    .prop-icon { width: 38px; height: 38px; transition: transform 0.2s; }
    .prop-icon.spinning { animation: spin 0.8s linear infinite; }
...
<body>
  ...
  <div class="navbar">
    ...
    <div class="nav-center">
      <span class="nav-center-text">MARUT THRUST STAND</span>
      <svg id="propIcon" viewBox="0 0 100 100" class="prop-icon">
        <circle cx="50" cy="50" r="38" fill="none" stroke="#ecf0f1" stroke-width="2" stroke-dasharray="80 40" stroke-linecap="round" transform="rotate(20 50 50)"/>
        <circle cx="50" cy="50" r="24" fill="none" stroke="#ecf0f1" stroke-width="1.5" stroke-dasharray="45 30" stroke-linecap="round" transform="rotate(-40 50 50)"/>
        <path d="M 43 47.5 Q 10 45 4 50 Q 10 55 43 52.5 Z" fill="#ecf0f1" />
        <path d="M 57 47.5 Q 90 45 96 50 Q 90 55 57 52.5 Z" fill="#ecf0f1" />
        <circle cx="50" cy="50" r="6" fill="#1e2b3c" stroke="#ecf0f1" stroke-width="2"/>
        <circle cx="50" cy="50" r="2" fill="#ecf0f1" />
      </svg>
    </div>
  ...
  <script>
    ...
    function tareLoadCell() {
      ...
      fetch('/tare?t=' + Date.now()).then(r => r.text()).then(res => {
    ...
    function updateMultipliers() {
      ...
      fetch('/cal?t=' + Date.now() + '&v=' + v + '&i=' + i).then(r => r.text()).then(res => {
    ...
    let recordIndex = 1;
    let lastArduinoIndex = -1; // Added to strictly prevent duplicate entries!
    ...
    function fetchData() {
      fetch('/data?t=' + Date.now()).then(r => r.text()).then(t => {
        ...
        let arduinoIndex = parseInt(p[0]); // The real hardware packet ID
        let rawTime = parseFloat(p[1]);
        
        document.getElementById('status_text').innerText = "Live | Uptime: " + rawTime.toFixed(1) + "s";
        
        // --- MATHEMATICAL OVERRIDE FOR ZERO-JITTER CSV LOGGING ---
        if (isRecording) {
          // Check if this is truly a new packet, not just a duplicate browser fetch
          if (arduinoIndex !== lastArduinoIndex) {
            lastArduinoIndex = arduinoIndex;
            
            // Mathematically lock the CSV time output to exactly 0.5s increments!
            let exactTime = ((recordIndex - 1) * 0.5).toFixed(1);
            
            let dataValues = [...p]; 
            dataValues[0] = recordIndex; // Enforce clean row number    
            dataValues[1] = exactTime;   // Enforce clean 0.5s math 
            
            csvLog.push(dataValues.join(','));
            document.getElementById('log_count').innerText = recordIndex;
            recordIndex++; 
          }
        }
        
        let v_thr = parseFloat(p[2]) || 0;
        let v_pwm = parseFloat(p[3]) || 0;
        let v_rpm = parseFloat(p[4]) || 0;
        let v_tht = parseFloat(p[5]) || 0;
        let v_amp = parseFloat(p[6]) || 0;
        let v_vol = parseFloat(p[7]) || 0;
        let v_pow = parseFloat(p[8]) || 0;
        let rawTemp = p[9].trim();
    ...
    function toggleRecord() {
      ...
      let prop = document.getElementById('propIcon');
      ...
      if (!isRecording) {
        ...
        csvLog = [csvHeader];
        recordIndex = 1;
        lastArduinoIndex = -1; // Reset memory tracker
    ...
    function restartSession() {
      ...
      fetch('/reset?t=' + Date.now()).then(r => r.text()).then(res => {
        isRecording = false;
  ...
  // ==========================================
  // --- 2. MAIN DAQ STRICT TIMING LOOP ---
  // ==========================================
  if (currentMillis - previousPrintMillis >= printInterval) {
    
    // --- PERFECT MATHEMATICAL TIME LOCK ---
    previousPrintMillis += printInterval; 
    
    // Anti-Rush: Drops missed loops if WiFi caused severe freezing
    if (currentMillis - previousPrintMillis >= printInterval) {
      unsigned long missedIntervals = (currentMillis - previousPrintMillis) / printInterval;
      previousPrintMillis += (missedIntervals * printInterval);
    }
    ...
    latestCSV = String(logIndex) + "," + 
                String(runTimeSeconds, 1) + "," + 
                String(throttlePercent, 0) + "," + 
                String(currentPWM) + "," + 
                rpmStr + "," + 
                String(currentThrust, 0) + "," + 
