...
void loop() {
  ...
  if (currentSerialState && !lastSerialState) {
    ...
    // Print pure CSV header to serial monitor for raw datalogging
        ...
        if (c == '\n') {
          if (currentLine.length() == 0) {
            ...
            if (requestType == 1) {
              ...
              client.println();
              client.print(latestCSV); // This is now a tiny, pure number string
            } 
            // --- ROUTE: RESET LOGGING ---
...
<body>
  ...
  <script>
    ...
    function fetchData() {
      fetch('/data?t=' + Date.now()).then(r => r.text()).then(t => {
        ...
        let p = t.split(',');
        if(p.length < 10) return; // We now expect exactly 10 raw numbers
        ...
        if (isRecording) {
          ...
          // CSV LOGGING FIX: The data array 'p' is already pure numbers!
          let dataValues = [...p]; 
          dataValues[0] = recordIndex;     // Replace pure index
          dataValues[1] = normalizedTime;  // Replace raw time with normalized time
        ...
        // --- JAVASCRIPT FIX: Parsing the pure number array directly ---
  ...
  if (currentMillis - previousPrintMillis >= printInterval) {
    ...
    float runTimeSeconds = (currentMillis - startTimeOffset) / 1000.0;

    String tempStr = (tempC <= -127.0) ? "ERR" : String(tempC, 1);
    String rpmStr = printRawRPM ? String(rawRPM, 1) : String(smoothedRPM, 1);

    // --- ARDUINO FIX: Tightly packed raw CSV data without any words ---
    ...
    if (currentSerialState) {
      Serial.println(latestCSV); // This will print clean, raw CSV data to the serial monitor!
    }
