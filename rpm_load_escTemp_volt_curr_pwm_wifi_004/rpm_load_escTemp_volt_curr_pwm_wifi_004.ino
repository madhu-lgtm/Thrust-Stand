...
void loop() {
  ...
  if (client) {
    String currentLine = "";
    int requestType = 0; // 0 = HTML UI, 1 = Data String, 2 = Reset Command
        ...
        if (c == '\n') {
          if (currentLine.length() == 0) {
            ...
            // --- ROUTE: SEND DASHBOARD UI ---
              ...
              // -----------------------------------------------------------------
              // PURE HTML/JS/CSS PAYLOAD WITH SIDEBAR
              // -----------------------------------------------------------------
              client.print(R"=====(
<!DOCTYPE html>
<html>
<head>
  ...
  <style>
    ...
    .sidebar h2 { color: #ecf0f1; font-size: 22px; margin-top: 0; margin-bottom: 40px; letter-spacing: 1px; font-weight: 600; border-bottom: 1px solid #34495e; padding-bottom: 15px;}
    .btn { background: #3498db; color: white; border: none; padding: 14px 15px; margin-bottom: 20px; border-radius: 8px; cursor: pointer; font-weight: 600; font-size: 15px; transition: 0.2s; display: flex; align-items: center; justify-content: center; }
...
<body>
  ...
  <div class="sidebar">
    ...
    <button class="btn btn-danger" onclick="restartSession()">
      Restart New Data
    </button>
  ...
  <script>
    ...
    // Memory to hold Excel-ready CSV data
    ...
    function fetchData() {
      fetch('/data').then(r => r.text()).then(t => {
        ...
        document.getElementById('status_text').innerText = "Live | Uptime: " + p[1].split(':')[1].trim() + "s";
        
        // Strip text labels and save clean numbers for Excel Download
        let cleanRow = p.map(item => item.split(':')[1].trim()).join(',');
        
        // Only add if it's a new row to prevent duplicate entries
        ...
        // Parse numbers for the UI Dashboard
        ...
        if (rawTemp === "ERR") {
          ...
          d.tmp.push(v_tmp); if(d.tmp.length > maxPts) d.tmp.shift(); drawSparkline('tmp', d.tmp, '#c0392b');
        }

        d.thr.push(v_thr); if(d.thr.length > maxPts) d.thr.shift(); drawSparkline('thr', d.thr, '#3498db'); 
        d.pwm.push(v_pwm); if(d.pwm.length > maxPts) d.pwm.shift(); drawSparkline('pwm', d.pwm, '#00bcd4'); 
        d.rpm.push(v_rpm); if(d.rpm.length > maxPts) d.rpm.shift(); drawSparkline('rpm', d.rpm, '#9b59b6'); 
        d.tht.push(v_tht); if(d.tht.length > maxPts) d.tht.shift(); drawSparkline('tht', d.tht, '#e74c3c'); 
        d.amp.push(v_amp); if(d.amp.length > maxPts) d.amp.shift(); drawSparkline('amp', d.amp, '#f39c12'); 
        d.vol.push(v_vol); if(d.vol.length > maxPts) d.vol.shift(); drawSparkline('vol', d.vol, '#2ecc71'); 
        d.pow.push(v_pow); if(d.pow.length > maxPts) d.pow.shift(); drawSparkline('pow', d.pow, '#e67e22'); 
    ...
    function downloadCSV() {
      if(csvLog.length <= 1) {
        alert("No data collected yet! Run the motor first.");
        return;
      }
    ...
    function restartSession() {
      if(!confirm("Are you sure? This will reset the Arduino clocks and clear your current data.")) return;
      
      fetch('/reset').then(r => r.text()).then(res => {
        // Clear local browser memory
        csvLog = [csvHeader];
        document.getElementById('log_count').innerText = "0";
        
        // Clear all graphs
        d = { thr:[], pwm:[], rpm:[], tht:[], amp:[], vol:[], pow:[], tmp:[] };
        ['thr','pwm','rpm','tht','amp','vol','pow','tmp'].forEach(id => drawSparkline(id, [], '#000'));
