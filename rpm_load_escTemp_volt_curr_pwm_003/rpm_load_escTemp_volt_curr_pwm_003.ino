...
void setup() {
  ...
  Serial.println("INITIALIZING STANDALONE MASTER DAQ (ANTI-DRIFT TIME LOCK)...");
  ...
  tempSensor.begin();
  // FAST-LOOP FIX: Drop resolution to 9-bit (fast) and don't block the loop!
...
void loop() {
  ...
  if (currentMillis - previousPrintMillis >= printInterval) {
    ...
    // ==========================================
    // 2. GATHER THRUST (FAST READ)
    // ==========================================
    float currentThrust = scale.get_units(1); // Reduced to 1 to prevent loop blocking
    if (currentThrust < 0) currentThrust = 0;

    // ==========================================
    // 3. GATHER TEMP (ASYNC)
    // ==========================================
    float tempC = tempSensor.getTempCByIndex(0); // Read the previously requested temp
    tempSensor.requestTemperatures(); // Ask sensor to start working on the NEXT reading
    ...
    // --- ANTI-DRIFT TIME CALCULATION ---
    // Instead of using raw 'millis', we use the perfect 500ms intervals
    previousPrintMillis += printInterval; 
    float runTimeSeconds = previousPrintMillis / 1000.0;
    ...
    Serial.print(" , Temp (°C): ");
    if (tempC <= -127.0) { // Catch the exact DS18B20 error code for disconnected sensor
      Serial.println("ERR"); 
    } else {
