        ...
        if (c == '\n') {
          if (currentLine.length() == 0) {
            ...
            // --- ROUTE: RESET LOGGING ---
              ...
              previousPrintMillis = millis(); // Reset sync timer instantly
              client.println("HTTP/1.1 200 OK");
              client.println("Connection: close");
              client.println();
...
<body>
  ...
  <script>
    ...
    // --- CACHE-BUSTING IMPLEMENTED ON ALL NETWORK REQUESTS ---
    ...
    function fetchData() {
      // Bypasses browser cache completely so you get true live data
    ...
    function restartSession() {
      ...
      // Cache buster ensures the browser always sends the Reset command to Arduino
      fetch('/reset?t=' + Date.now()).then(r => r.text()).then(res => {
        ...
        document.getElementById('status_text').innerText = "Session Reset!";
            ...
            // Because of startsWith, the Cache-buster query parameters (?t=123...) don't break the route mapping!
  ...
  if (currentMillis - previousPrintMillis >= printInterval) {
    ...
    // --- DAQ TIMER CATCH-UP FIX ---
    // Forces a strict delay interval so Arduino doesn't try to rapidly "catch up" on missed prints if busy
    previousPrintMillis = currentMillis; 
    ...
    // Uses absolute time difference to calculate accurate run time, preventing jumps
