...
// --- PIN DEFINITIONS ---
// ==========================================
const int pwmInputPin = 2;       // Reads the ESC PWM signal
const int rpmPin = 3;            // Reads the optical RPM sensor
...
volatile unsigned int sharedPWM = 0; // Value shared between interrupt and main loop
...
void setup() {
  ...
  Serial.println("INITIALIZING STANDALONE MASTER DAQ (ZERO-LAG + PWM MODE)...");
...
void loop() {
  ...
  if (currentMillis - previousPrintMillis >= printInterval) {
    ...
    // ==========================================
    // 1. GATHER INTERRUPT DATA (RPM & PWM)
    // ==========================================
    noInterrupts(); // Pause safely to grab volatile variables
    ...
    interrupts(); // Resume normal operations
...
// RPM Reader (Fires when magnet passes)
void measurePulse() {
...
// PWM Reader (Fires when signal shifts high or low)
void measurePWM() {
  unsigned long currentTime = micros();
  // If the pin just went HIGH, start the stopwatch
  if (digitalRead(pwmInputPin) == HIGH) {
    pwmStartTime = currentTime;
  } 
  // If the pin just went LOW, stop the stopwatch and calculate the width
  else {
    unsigned int pulseWidth = currentTime - pwmStartTime;
    // Standard RC PWM is between 900µs and 2100µs. 
    // This filter ignores random noise spikes.
